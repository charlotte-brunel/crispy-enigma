...
//------------------------------------------------------------------------------------------------------------
void generation_kmer(int position_kmer, char* k_mer, TPtr_dictionnaire_sequences* adr_tete_dict_seq, TPtr_Cellkmer* adr_tete_CellKmer, TPtr_CellSequence* adr_tete_CellSequenceK, TPtr_CellPos*adr_tete_CellPosK)
{
	TPtr_dictionnaire_sequences p_dictionnaire_sequence = *adr_tete_dict_seq;
  TPtr_Cellkmer p_kmer = *adr_liste_kmer; //on créer des pointeurs temporaires pour parcourir les listes
	TPtr_Cellkmer p_prec_kmer = NULL;


  if (p_kmer->suiv_kmer == NULL) //cas du premier element de la liste
  {
    ...
    p_kmer->suiv_kmer = nouveau_kmer;
    // p_kmer->tete_sequence = nouvelle_sequence;
    ...
    while (p_kmer->suiv_kmer != NULL)
    {
      if (strcmp(p_kmer->kmer, k_mer) == 0)   //si le kmer a d�j� �t� trouv�
      {
        TPtr_CellSequence p_liste_sequence = p_kmer->tete_sequence;
...
void parcours_masque( void* adr_masque, TPtr_dictionnaire_sequences* adr_tete_dict_seq, TPtr_Cellkmer* adr_tete_CellKmer, TPtr_CellSequence* adr_tete_CellSequenceK, TPtr_CellPos* adr_tete_CellPosK)
{
  ...
  while (p_dictionnaire_sequence != NULL) //parcourt du dictionnaire de séquence
  {
    while (position < 30) //inferieur à longueur de la séquence
    {
      ...
      generation_kmer(position_kmer, k_mer, &p_dictionnaire_sequence, &p_kmer, &p_sequence, &p_pos);
    }
    position = 0;
