...
int main()
{
  ...
  int nb_sequence = 10; // nombre de séquence à générer
  int i,j; //itérateur de boucle
  char nucleo = ' ';

  ptr_struct_seq element_generation_sequence = malloc(sizeof(structure_sequence)); // On créer le premier élément de la structure
  ...
  // Début du code:
  printf("aaaaaaargh");
  for(i=1; i <= nb_sequence; i++) //Boucle qui permet de créer les 10 séquences du fichier fasta
  {
  	element_generation_sequence->numero_sequence = i;
  	for (j=0; j < 30; j++) //boucle qui permet de créer les séquences de 30 nucléotides de long
    {
      random_nucleotide(&nucleo);
      element_generation_sequence->sequence[j] = nucleo;
  		// on utilise la fonction random_nucleotide pour choisir aléatoirement les 30 nucléotides.
  	}
  	element_generation_sequence->sequence[30] = '\0';
  	if(i != 10)  // Tant qu'on est pas au dernier élément on continue de créer des briques
    {
  		element_generation_sequence_next = malloc(sizeof(structure_sequence)); //on créer un élément de type structure_séquence
  		element_generation_sequence-> next_sequence = element_generation_sequence_next; // le pointeur de la chaque brique pointe sur l'élément suivant
  		element_generation_sequence = element_generation_sequence_next; //on passe de la brique fraîchement créée à la suivante
  	}else{ //cas du dernier élément de la liste
